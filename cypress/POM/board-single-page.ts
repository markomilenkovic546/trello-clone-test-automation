    ...
    constructor() {
        ...
        this.elements = {
            ...
            boardSettingsModal: {
                ...
                deleteButton: () =>
                    cy.get(".chakra-button.css-1w3ruwl:contains('Delete')"),
            ...
            customizeBoardModal: {
                ...
                imageItem: (index: any) =>
                    cy.get(`.css-1hdm3lc div:nth-of-type(${index})`),
            ...
            column: {
                columnItem: (columnName: string) =>
                    cy.get(`.css-4e3rfe:contains('${columnName}')`),
                columnItems: () => cy.get('.css-4e3rfe'),
                columnHeader: (columnName: string) =>
                    cy.get(`.css-1lekzkb:contains('${columnName}')`),
                columnTitle: (columnName: string) =>
                    cy.get(`.css-k008qs:contains('${columnName}')`),
                manageColumnButton: (columnName: string) =>
                    cy
                        .get(`.css-1lekzkb:contains('${columnName}')`)
                        .find('button[aria-label="Options"]'),
                editColumnButton: (columnName: string) =>
                    this.elements.column
                        .columnItem(columnName)
                        .find('[role="menuitem"]')
                        .contains('Edit'),
                deleteColumnButton: (columnName: string) =>
                    this.elements.column
                        .columnItem(columnName)
                        .find('[role="menuitem"]')
                        .contains('Delete'),
            ...
            card: {
                cardItem: (columnName: string, cardName: string) =>
                    cy
                        .get(`.css-4e3rfe:contains('${columnName}')`)
                        .find(`.css-1g2hcr8:contains('${cardName}')`),
                ...
                editCardModal: {
                    ...
                    labelOption: (index: any) =>
                        cy.get(`.css-163woa2:nth-of-type(${index})`),
                    assingToButton: () => cy.get('button:contains("Assign To")'),
                    assingOption: (memberName: string) =>
                        cy.get(`button:contains("${memberName}")`),
        ...
        this.actions = {
            header: {
                ...
                clickOnProfileButton: () =>
                    this.elements.header.profileButton().click(),
                clickOnInviteButton: () => this.elements.header.inviteButton().click(),
                clickOnSettingsButton: () =>
                    this.elements.header.settingsButton().click(),
                clickOnCustomizeBoardsButton: () =>
                    this.elements.header.customizeBoardButton().click()
            },

            profileDropDown: {
                clickOnLogOutButton: () =>
                    this.elements.prifileDropDown.logOutButton().click()
            },

            boardSettingsModal: {
                clickOnBasicTab: () =>
                    this.elements.boardSettingsModal.basicTab().click(),
                clickOnAdvanceTab: () =>
                    this.elements.boardSettingsModal.advanceTab().click(),
                typeBoardName: (boardName: string) =>
                    this.elements.boardSettingsModal.boardNameFiled().type(boardName),
                clearBoardName: () =>
                    this.elements.boardSettingsModal.boardNameFiled().clear(),
                clickOnSaveButton: () =>
                    this.elements.boardSettingsModal.saveButton().click(),
                clickOnDeleteButton: () =>
                    this.elements.boardSettingsModal.deleteButton().click(),
                clickOnCloseButton: () =>
                    this.elements.boardSettingsModal.closeModalButton().click()
            },

            inviteUserModal: {
                typeEmail: (email: string) =>
                    this.elements.inviteUserModal.emailField().type(email),
                clickOnInviteButton: () =>
                    this.elements.inviteUserModal.inviteButton().click(),
                clickOnCloseModalButton: () =>
                    this.elements.inviteUserModal.closeModalButton().click()
            },

            customizeBoardModal: {
                typeSearchText: (searchText: string) =>
                    this.elements.customizeBoardModal.searchField().type(searchText),
                clickOnSearchButton: () =>
                    this.elements.customizeBoardModal.searchButton().click(),
                clickOnImageItem: (index: any) =>
                    this.elements.customizeBoardModal.imageItem(index).click(),
                clickOnLoadMoreButton: () =>
                    this.elements.customizeBoardModal.loadMoreButton().click(),
                clickOnSaveButton: () =>
                    this.elements.customizeBoardModal.saveButton().click(),
                clickOnCloseModalButton: () =>
                    this.elements.customizeBoardModal.closeModalButton().click()
            },

            column: {
                clickOnColumnItem: (columnName: string) =>
                    this.elements.column.columnItem(columnName).click(),
                clickOnManageColumnButton: (columnName: string) =>
                    this.elements.column.manageColumnButton(columnName).click(),
                clickOnEditColumnButton: (columnName: string) =>
                    this.elements.column
                        .editColumnButton(columnName)
                        .click({ force: true }),
                clickOnDeleteColumnButon: (columnName: string) =>
                    this.elements.column
                        .deleteColumnButton(columnName)
                        .click({ force: true }),
                clickColumnTitleField: (columnName: string) =>
                    this.elements.column.columnTitleField(columnName).click(),
                clickOnAddCardButton: (columnName: string) =>
                    this.elements.column.addCardButton(columnName).click(),
                clickOnAddColumnButton: () =>
                    this.elements.column.addColumnButton().click(),
                clearColumnName: (columnName: string) =>
                    this.elements.column.columnTitleField(columnName).clear(),
                typeColumnName: (columnName: string, text: string) =>
                    this.elements.column
                        .columnTitleField(columnName)
                        .type(`${text}{enter}`)
            },
            ...
            card: {
                ...
                editCardModal: {
                    typeCardName: (cardName: string) =>
                        this.elements.card.editCardModal.cardNameField().type(cardName),
                    clearCardName: () =>
                        this.elements.card.editCardModal.cardNameField().clear(),
                    typeDescription: (description: string) =>
                        this.elements.card.editCardModal
                            .descriptionTextBox()
                            .type(description),
                    clearDescription: () =>
                        this.elements.card.editCardModal.descriptionTextBox().clear(),
                    clickOnLabelsButton: () =>
                        this.elements.card.editCardModal.labelsButton().click(),
                    clickOnLabelOption: (index: any) =>
                        this.elements.card.editCardModal.labelOption(index).click(),
                    clickOnAssingToButton: () =>
                        this.elements.card.editCardModal.assingToButton().click(),
                    clickOnAssignOption: (memberName: string) =>
                        this.elements.card.editCardModal
                            .assingOption(memberName)
                            .click(),
                    clickOnDeleteButton: () =>
                        this.elements.card.editCardModal.deleteButton().click(),
                    clickOnCloseModalButton: () =>
                        this.elements.card.editCardModal.closeModalButton().click()
                }
            }
        };
    }
