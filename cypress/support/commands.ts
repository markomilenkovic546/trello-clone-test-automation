...
Cypress.Commands.add(
    ...
    (email: string, name: string, password: string, confirmPassword: string) => {
        ...
        registrationPage.elements
            .emailField()
            .should('have.attr', 'value', email + randomString);
...
Cypress.Commands.add(
    'verifyInviteUserRequest',
    (interception: any, user: any, board: any) => {
...
Cypress.Commands.add('verifyEmailAndAcceptInvitation', (email: string) => {
    cy.request('GET', `https://api.testsendr.link/?email=${email}`).then((response) => {
        ...
        expect(responseBody[0].subject).to.equal(
            'You are invited to join to a trello clone board'
        );
