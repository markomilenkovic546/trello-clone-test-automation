...
beforeEach(function () {
    ...
    if (
        this.currentTest.title === 'User can sign up with valid credentials' ||
        this.currentTest.title === 'User can log in with valid credentials'
    ) {
        return;
    }
    cy.visit('/');
...
describe('Smoke tests', () => {
    ...
    it('User can log in with valid credentials', function () {
        ...
        loginPage.elements
            .passwordField()
            .should('have.attr', 'value', users[1].Password);
        // Click on the submit button
    ...
    it('User can create a new board', function () {
        cy.intercept('POST', ' https://trello-clone-one.vercel.app/api/boards').as(
            'createBoard'
        );
        //Click on the "Boards" button
        sideNavigation.actions.clickOnBoardsButton();
