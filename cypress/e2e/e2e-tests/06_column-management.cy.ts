...
describe('Tests which cover column management', () => {
    ...
    it('User can edit a column name', function () {
        ...
        boardSinglePage.actions.column.clickOnManageColumnButton(
        ...
        boardSinglePage.actions.column.typeColumnName(
            columns[1].Column_Name,
            'EditedColumn'
        );
        // Verify that only expected column name is edited
        boardSinglePage.elements.column
            .columnItem(columns[1].Column_Name + 'EditedColumn')
            .should('have.length', 1);
    });
    ...
    it('User can delete a column', function () {
        ...
        boardSinglePage.elements.column
            ...
            .then((length) => {
                // Click on the manage column button
                boardSinglePage.actions.column.clickOnManageColumnButton(
                    columns[2].Column_Name
                );
                // Click on the Delete button
                boardSinglePage.actions.column.clickOnDeleteColumnButon(
                    columns[2].Column_Name
                );
                // Verify that expected colum is deleted
                boardSinglePage.elements.column
                    .columnItem(columns[2].Column_Name)
                    .should('not.exist');
    ...
    it('User can move a column', function () {
        ...
        boardSinglePage.elements.header.boardTitle().contains(boards[0].Board_Name);
        boardSinglePage.elements.column
            .columnTitle(columns[4].Column_Name)
            .as('source');
