...
describe("Tests which cover card management", function () {
  ...
  it("User can edit a card description", function () {
    ...
    boardSinglePage.elements.header.boardTitle().contains(boards[0].Board_Name);
    // Click on the card
    boardSinglePage.actions.card.clickOnCardItem(columns[0].Column_Name, cards[3].Card_Name);
    // Clear description text box
    boardSinglePage.actions.card.editCardModal.clearDescription();
    // Type description
    boardSinglePage.actions.card.editCardModal.typeDescription("Edited description");
    ...
    boardSinglePage.elements.card.editCardModal.descriptionTextBox().should("have.text", "Edited description");
  });
  ...
  it("User can set a label on the card", function () {
    ...
    boardSinglePage.elements.card.cardItem(columns[0].Column_Name, cards[3].Card_Name).find("span").contains("performance");
  });
  ...
  it("User can asign member to the card", function () {
    ...
    boardSinglePage.actions.card.editCardModal.clickOnAssignOption(boards[0].Board_Owner);
    ...
    cy.extractMemberInitials(boards[0].Board_Owner).then((initials) => {
      ...
      boardSinglePage.elements.card.cardItem(columns[0].Column_Name, cards[3].Card_Name).contains(initials);
    });
  });
  ...
  it("User can edit a card title", function () {
    ...
    boardSinglePage.actions.card.clickOnCardItem(columns[0].Column_Name, cards[3].Card_Name + "edited");
  });
  it("User can delete a card", function () {
    ...
    boardSinglePage.elements.card
      ...
      .then((length) => {
        ...
        // Click on the card
        boardSinglePage.actions.card.clickOnCardItem(columns[0].Column_Name, cards[2].Card_Name);
        // Click on the delete button
        boardSinglePage.actions.card.editCardModal.clickOnDeleteButton();
        // Verify that expected card is deleted
        boardSinglePage.elements.card.cardItem(columns[0].Column_Name, cards[2].Card_Name).should("not.exist");
  ...
  it("User can move a card by drag&drop", function () {
    ...
    // Verify that Board title is correct
    boardSinglePage.elements.header.boardTitle().contains(boards[0].Board_Name);
    boardSinglePage.elements.card.cardItem(columns[0].Column_Name, cards[0].Card_Name).as("source");
